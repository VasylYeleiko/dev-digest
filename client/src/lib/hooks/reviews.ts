...
// ---- Query keys: one factory for every reviews-area cache entry ----
// Module-private on purpose — callers invalidate through hooks
// (`useInvalidateRunState`), never by rebuilding raw key arrays.
type PrKey = string | null | undefined;
const reviewKeys = {
  activeRuns: (prId: PrKey) => ["pr-active-runs", prId] as const,
  runs: (prId: PrKey) => ["pr-runs", prId] as const,
  reviews: (prId: PrKey) => ["reviews", prId] as const,
  comments: (prId: PrKey) => ["pr-comments", prId] as const,
};

/** Refetch a PR's run state after SSE says a run started or settled. */
export function useInvalidateRunState(prId: PrKey) {
  const qc = useQueryClient();
  return React.useMemo(
    () => ({
      /** In-flight runs (the live-status pills). */
      activeRuns: () => {
        if (prId) qc.invalidateQueries({ queryKey: reviewKeys.activeRuns(prId) });
      },
      /** Full run history — so a just-failed run shows up without a reload. */
      history: () => {
