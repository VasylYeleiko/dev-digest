...
- **Data hooks** — one file per API resource under `src/lib/hooks/`
  ...
  (`usePulls`, `useFindingAction`). Each file owns one query-key factory
  (`reviewKeys`, `coreKeys`, …); components never build raw key arrays —
  cross-component refreshes go through a hook (`useInvalidateRunState`).
...
- `src/vendor/ui` — vendored copy of `@devdigest/ui`, edit the source package
  instead.
- `src/vendor/shared` — a mirror of `server/src/vendor/shared/contracts`: change
  a contract there first, then copy it here in the same change.
