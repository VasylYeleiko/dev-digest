...
engine mock-testable. `npm run build` is a type-check only — the package never
...
- `npm run typecheck` — src + tests (`tsconfig.test.json`)
...
- **Contracts are imported, never redefined.** `Review`, `Finding`, `Verdict`
  ...
  them.
- Tests are hermetic only (`npm test`, stubbed `LLMProvider`) — there is no
