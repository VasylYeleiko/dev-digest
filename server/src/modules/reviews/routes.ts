...
import { createReviewService } from './compose.js';
...
export default async function reviewsRoutes(appBase: FastifyInstance) {
  ...
  const service = createReviewService(app.container, app.log);

  // ---- Run a review (manual trigger) -------------------------------
  // Tight per-route limit: each call can fan out to expensive LLM runs.
  // Every body field is optional and an empty body is OK — hence `.nullish()`,
  // since Fastify hands an empty body to the validator as null.
  app.post(
    '/pulls/:id/review',
    {
  ...
  app.get(
    ...
    reply.sse(
      (async function* () {
        for await (const e of service.events(workspaceId, runId)) {
  ...
  app.post('/runs/:id/cancel', { schema: { params: IdParams } }, async (req) => {
    const { workspaceId } = await ctx(req);
    const ok = await service.cancelRun(workspaceId, req.params.id);
    return { ok };
  });
