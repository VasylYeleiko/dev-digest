...
import { createSettingsService } from '../src/modules/settings/compose.js';
...
d('Settings: feature models + secrets status (Testcontainers pg)', () => {
  ...
  it('resolveFeatureModel: registry default until overridden, then the workspace choice', async () => {
    ...
    const settings = createSettingsService(app.container);

    // No override yet → registry default; featureModelOverride is undefined.
    expect(await settings.featureModelOverride(workspaceId, 'onboarding')).toBeUndefined();
